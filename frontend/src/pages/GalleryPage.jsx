...
export default function GalleryPage() {
  ...
  function handleFileSelect(e) {
    ...
    var valid = files.filter(function(f) {
      var validTypes = ["image/jpeg", "image/png", "image/heic", "image/heif"];
      if (validTypes.indexOf(f.type) === -1) { toast.error(f.name + " nu este valid"); return false; }
  ...
  function removeFile(idx) {
    var newFiles = uploadFiles.filter(function(_, i) { return i !== idx; });
    ...
    <div className="min-h-screen bg-[#F9F7F2]">
        ...
        <div className="max-w-6xl mx-auto">
          ...
          {loading ? (
            ...
            <div className="text-center py-20 bg-white border border-[#2C3E30]/5">
              ...
              <button onClick={function() { setShowUpload(true); }} className="inline-flex items-center gap-2 px-8 py-4 bg-[#2C3E30] text-[#F9F7F2] font-serif" data-testid="empty-upload-btn"><Upload size={20} /> Incarca Prima Poza</button>
            </div>
          ) : (
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              {photos.map(function(photo) {
                return (
                  <div key={photo.id} className="aspect-square overflow-hidden cursor-pointer" onClick={function() { setSelectedPhoto(photo); }} data-testid={"photo-" + photo.id}>
      ...
      {showUpload && (
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
          <div className="bg-[#F9F7F2] max-w-lg w-full p-8 relative">
            <button onClick={function() { setShowUpload(false); }} className="absolute top-4 right-4 text-[#5C6B5F]"><X size={24} /></button>
            <h2 className="font-serif text-2xl text-[#2C3E30] mb-6">Incarca Poze</h2>
            <div className="space-y-6">
              <div>
                <label className="text-[#2C3E30] text-sm tracking-widest uppercase mb-2 block">Numele tau (optional)</label>
                <input type="text" value={uploadName} onChange={function(e) { setUploadName(e.target.value); }} className="w-full p-3 border border-[#2C3E30]/20" placeholder="ex: Maria Ionescu" data-testid="upload-name-input" />
              </div>
              <div>
                ...
                <button onClick={function() { fileInputRef.current && fileInputRef.current.click(); }} className="w-full p-8 border-2 border-dashed border-[#2C3E30]/20 hover:border-[#8DA399] text-center" data-testid="select-files-btn">
                ...
                {uploadFiles.length > 0 && (
                  <div className="mt-4 space-y-2">
                    {uploadFiles.map(function(file, i) {
                      return (
                        <div key={i} className="flex items-center justify-between p-2 bg-white border border-[#2C3E30]/5">
                          ...
                          <button onClick={function() { removeFile(i); }} className="text-[#B95D5D]"><X size={16} /></button>
                        </div>
                      );
                    })}
                  </div>
              ...
              <div className="flex items-start gap-3">
                <input type="checkbox" id="consent" checked={uploadConsent} onChange={function(e) { setUploadConsent(e.target.checked); }} className="mt-1" data-testid="consent-checkbox" />
              ...
              <div className="flex gap-4">
                <button onClick={function() { setShowUpload(false); }} className="flex-1 px-6 py-3 border border-[#2C3E30] text-[#2C3E30] font-serif hover:bg-[#2C3E30] hover:text-[#F9F7F2]" data-testid="cancel-upload-btn">Anuleaza</button>
                <button onClick={handleUpload} disabled={uploading || !uploadConsent || uploadFiles.length === 0} className="flex-1 px-6 py-3 bg-[#2C3E30] text-[#F9F7F2] font-serif disabled:opacity-50 flex items-center justify-center gap-2" data-testid="confirm-upload-btn">{uploading ? "Se incarca..." : "Incarca"}</button>
              </div>
            </div>
          </div>
        </div>
      ...
      {selectedPhoto && (
        <div className="fixed inset-0 z-50 bg-black/90 flex items-center justify-center p-4" onClick={function() { setSelectedPhoto(null); }}>
          <button className="absolute top-4 right-4 text-white hover:text-[#8DA399]" onClick={function() { setSelectedPhoto(null); }} data-testid="close-lightbox-btn"><X size={32} /></button>
          <img src={API + "/photos/file/" + selectedPhoto.filename} alt={selectedPhoto.original_filename} className="max-w-full max-h-[90vh] object-contain" onClick={function(e) { e.stopPropagation(); }} />
        </div>
      )}
    </div>
  );
