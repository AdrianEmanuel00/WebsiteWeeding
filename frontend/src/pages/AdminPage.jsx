...
export default function AdminPage() {
  ...
  function loadData(token) {
    ...
    Promise.all([
      ...
      setData({ stats: results[0].data, rsvps: results[1].data, photos: results[2].data });
  ...
  function login(e) {
    e.preventDefault();
    setLoading(true);
    axios.post(API + "/admin/login", { username: user, password: pass }).then(function(r) {
      localStorage.setItem("admin_token", r.data.token);
      setAuth(true);
      loadData(r.data.token);
      toast.success("OK");
    }).catch(function() { toast.error("Invalid"); }).finally(function() { setLoading(false); });
  }

  function logout() { localStorage.removeItem("admin_token"); setAuth(false); setData(null); }

  function moderate(id, status) {
    var t = localStorage.getItem("admin_token");
    axios.put(API + "/photos/" + id + "/moderate", { status: status }, { headers: { Authorization: "Bearer " + t }}).then(function() { loadData(t); toast.success("Done"); });
  }

  function del(type, id) {
    var t = localStorage.getItem("admin_token");
    axios.delete(API + "/" + type + "/" + id, { headers: { Authorization: "Bearer " + t }}).then(function() { loadData(t); toast.success("Deleted"); });
  }
    ...
    return (
      <div className="min-h-screen bg-[#F9F7F2] flex items-center justify-center px-6">
        <div className="max-w-md w-full">
          <div className="text-center mb-8"><Link to="/" className="font-serif text-3xl text-[#2C3E30]">S & A</Link><h1 className="font-serif text-2xl text-[#2C3E30] mt-4">Panou Admin</h1></div>
          <form onSubmit={login} className="bg-white p-8 border border-[#2C3E30]/5 space-y-6">
            <div><label className="text-[#2C3E30] text-sm tracking-widest uppercase mb-2 block">Utilizator</label><input type="text" value={user} onChange={function(e) { setUser(e.target.value); }} className="w-full p-3 border border-[#2C3E30]/20" required data-testid="admin-username" /></div>
            <div><label className="text-[#2C3E30] text-sm tracking-widest uppercase mb-2 block">Parola</label><input type="password" value={pass} onChange={function(e) { setPass(e.target.value); }} className="w-full p-3 border border-[#2C3E30]/20" required data-testid="admin-password" /></div>
    ...
    <div className="min-h-screen bg-[#F9F7F2]">
      <header className="bg-[#2C3E30] text-[#F9F7F2] px-6 py-4">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-4"><Link to="/" className="font-serif text-xl">S & A</Link><span className="text-[#8DA399]">|</span><span className="text-sm uppercase">Admin</span></div>
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="bg-white p-6 border border-[#2C3E30]/5 mb-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">{Object.keys(data.stats.menu_counts).map(function(m) { return <div key={m} className="text-center p-4 bg-[#F0EFEA]"><div className="font-serif text-2xl text-[#8DA399]">{data.stats.menu_counts[m]}</div><div className="text-sm text-[#5C6B5F]">{m}</div></div>; })}</div>
        </div>
          ...
          <div className="bg-white border border-[#2C3E30]/5">
            ...
            <div className="overflow-x-auto">
              <table className="admin-table">
                ...
                <tbody>{data.rsvps.map(function(r) { return (<tr key={r.id}><td className="font-medium">{r.guest_name}</td><td className={r.attending ? "text-[#8DA399]" : "text-[#B95D5D]"}>{r.attending ? "Da" : "Nu"}</td><td>{r.num_guests}</td><td><ul className="text-sm">{r.guests.map(function(g, i) { return <li key={i}>{g.name}: {g.menu_type}</li>; })}</ul></td><td className="text-sm">{r.allergies || "-"}</td><td><button onClick={function() { del("rsvp", r.id); }} className="text-[#B95D5D]" data-testid={"delete-rsvp-" + r.id}>Sterge</button></td></tr>); })}</tbody>
              </table>
        ...
        {tab === "photos" && (
          <div className="bg-white border border-[#2C3E30]/5 p-4">
            {data.photos.length === 0 ? <div className="text-center py-12 text-[#5C6B5F]">Nicio poza</div> : (
              <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
                {data.photos.map(function(photo) { return (
